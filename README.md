...
- **Navidrome** — optional **Navidrome** tab with the same UX as Jellyfin, backed by the [Subsonic API](http://www.subsonic.org/pages/api.jsp); compatible with Navidrome, Airsonic, Subsonic and any Subsonic-compatible server. The tab only appears when `navidrome_url` is set in `config.toml`
...
To enable the Navidrome tab, add to `config.toml`:
...
Works with any [Subsonic-compatible](http://www.subsonic.org/pages/api.jsp) server: Navidrome, Airsonic, Subsonic, Gonic, etc. The tab appears only when `navidrome_url` is set. Authentication uses hex-encoded password (`p=enc:...`) — no MD5 dependency.

**Same limitations as Jellyfin:** read-only, no gapless, no album art from server.
...
![lavanda — internet radio](assets/radio.png)

*Radios tab — search, Top, SomaFM and per-country browsing, manual stream URL entry, codec/bitrate badges, live ICY "now playing" and favorites.*
...
├── navidrome.rs        # Subsonic API client for Navidrome and compatible servers
├── locale.rs           # i18n strings (en, pt_BR, es)
├── audio/
│   ├── player.rs       # symphonia decode + cpal output (dedicated thread)
│   ├── stream.rs       # Icecast/Shoutcast HTTP streaming + ICY metadata
